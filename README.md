...
2. Run `make hn`, costs nothing, takes 10 seconds, writes today's `scan-results/hn-scan-<date>.md` (or does nothing if it's already there), open it in your editor. Clickable links, a checkbox per lead. After you apply somewhere, tick its box (`- [ ]` to `- [x]`) and save, that file won't be touched again until a new day's scan creates the next one.
