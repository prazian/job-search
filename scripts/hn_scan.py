...
Writes a dated, clickable markdown report to scan-results/YYYY-MM-DD/hn-scan.md,
one folder per day (every source's report for that day lands in the same
folder, sorted naturally). If today's folder already has this file, running
this again does nothing to it, prints a note and exits, your edits are never
touched or re-parsed mid-day. Pass --force to rescan and merge fresh HN data
into today's file anyway. Each lead gets a checkbox, "- [ ]"/"- [x]".
...
If the HN fetch fails partway through, the whole run aborts and no file gets
written, nothing partial/broken persisted.
...
def bucket_matches(
    ...
    for m in matches:
        ...
        if prev_tag:
            skipped.append((m, _common.resolve_reason(prev_tag, None)))
...
def main():
    ...
    ap.add_argument("--out", default=None, help="markdown report path (default scan-results/<today>/hn-scan.md)")
